...
<div class="salary-container" style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); min-height: 100vh; padding: 30px 0;" dir="{{ $isAr ? 'rtl' : 'ltr' }}">
        ...
        <div class="row">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        <h1 class="h2 fw-bold text-dark mb-1" style="letter-spacing: -0.5px;">⏰ {{ __('admin.overtime') }}</h1>
                        <p class="text-muted mb-0" style="font-size: 14px;">{{ __('admin.salary_management_subtitle') }}</p>
                    </div>
                    <a href="{{ route('admin.overtimes.create') }}" class="btn fw-semibold" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; padding: 10px 20px; border-radius: 8px; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.3);">
                        <i class="bi bi-plus-lg"></i> {{ __('admin.add_overtime') }}
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="container-xl px-4 mb-4">
        <ul class="nav nav-tabs nav-fill gap-2 mb-4" role="tablist" style="background: white; padding: 8px 12px; border-radius: 10px; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.1);">
            ...
            <li class="nav-item">
                <a class="nav-link fw-semibold active" href="{{ route('admin.overtimes.index') }}"
    ...
    <div class="container-xl px-4">
        {{-- FILTERS --}}
        <form method="GET" action="{{ route('admin.overtimes.index') }}" class="row g-2 mb-3">
            <div class="col-md-4">
            ...
            <div class="col-md-3 d-flex align-items-end">
                ...
                <a href="{{ route('admin.overtimes.index') }}" class="btn btn-secondary">{{ __('admin.reset') }}</a>
            </div>
        </form>
                    ...
                    <table class="table mb-0" style="font-size: 13px;">
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom: 1px solid #f0f2f7; transition: all 0.2s; background: white;">
                                <td style="padding: 16px; color: #2c3e50; font-weight: 600;">{{ $ot->employee?->name ?? '-' }}</td>
                                <td style="padding: 16px; color: #27ae60; font-weight: 600;">{{ number_format($ot->amount, 2) }}</td>
                                <td style="padding: 16px; color: #667eea;">{{ $ot->date }}</td>
                                <td style="padding: 16px; color: #555;">{{ $ot->notes ?? '-' }}</td>
                                <td style="padding: 16px; text-align: center;">
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="{{ route('admin.overtimes.edit', $ot->id) }}" class="btn btn-sm btn-warning" style="padding: 6px 12px; border-radius: 6px;">
                                            <i class="bi bi-pencil"></i> {{ __('admin.edit') }}
                                        </a>
                                        <form action="{{ route('admin.overtimes.destroy', $ot->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
