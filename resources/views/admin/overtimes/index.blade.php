...
<div dir="{{ $isAr ? 'rtl' : 'ltr' }}">
    ...
    <div class="payroll-tabs">
        ...
        <a href="{{ route('admin.overtimes.index') }}" class="payroll-tab active">
    ...
    <div class="card mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('admin.overtimes.index') }}" class="d-flex align-items-end gap-3 flex-wrap">
    ...
    @if(session('success'))
        <div class="p-3 mb-4" style="background: #f0fdf4; color: #166534; border: none; border-left: 4px solid #22c55e; border-radius: 8px;">
                ...
                <div class="table-responsive">
                    <table class="table table-styled mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="td-name">{{ $ot->employee?->name ?? '-' }}</td>
                                <td class="td-pos">+{{ number_format($ot->amount, 2) }}</td>
                                <td class="td-accent">{{ $ot->date }}</td>
                                <td class="td-muted">{{ $ot->notes ?? '-' }}</td>
                                <td class="td-center">
                                    <a href="{{ route('admin.overtimes.edit', $ot->id) }}" class="btn btn-warning btn-sm">
                                        <i class="fas fa-edit me-1"></i>{{ __('admin.edit') }}
                                    </a>
                                    <form action="{{ route('admin.overtimes.destroy', $ot->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
