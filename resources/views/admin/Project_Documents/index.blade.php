...
<div class="container mt-4" dir="{{ $isAr ? 'rtl' : 'ltr' }}">
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.project-documents.index') }}" method="GET" class="d-flex align-items-end gap-4 flex-wrap">
                <div style="flex: 1; min-width: 250px;">
                    <label class="form-label fw-bold" style="color: #2c3e50; font-size: 13px; text-transform: uppercase; letter-spacing: 1px;">📅 {{ __('Filter by month') }}</label>
                ...
                <div style="flex: 1; min-width: 200px;">
                    <label class="form-label fw-bold" style="color: #2c3e50; font-size: 13px; text-transform: uppercase; letter-spacing: 1px;">📆 {{ __('Filter') }}</label>
                ...
                <div style="min-width: 120px;">
                    ...
                    <a href="{{ route('admin.project-documents.index') }}" class="btn btn-outline-secondary w-100" style="padding: 12px 14px; border: 2px solid #6c757d; border-radius: 8px; font-weight: 500; font-size: 14px;">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center">No project updates found for {{ request('month') ? \Carbon\Carbon::createFromFormat('Y-m', request('month'))->format('F Y') : __('All Data') }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>
