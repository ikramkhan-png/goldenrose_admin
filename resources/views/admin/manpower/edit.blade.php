...
@section('content')
    ...
    <div class="container" dir="{{ $isAr ? 'rtl' : 'ltr' }}">
        ...
        <h1 class="mb-4">{{ __('manpower.edit_manpower') }}</h1>
        ...
        <form action="{{ route('admin.manpower.update', $manpower->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" name="hourly_rate"
                    value="{{ old('hourly_rate', $manpower->hourly_rate) }}" class="form-control">
            </div>

            <div class="mb-3">
                <label class="form-label">{{ __('manpower.daily_rate') }}</label>
                <input type="number" step="0.01" name="daily_rate"
                    value="{{ old('daily_rate', $manpower->daily_rate) }}" class="form-control" required>
            </div>

            <div class="mb-3">
                <label class="form-label">{{ __('manpower.monthly_rate') }}</label>
                <input type="number" step="0.01" name="monthly_rate"
                    value="{{ old('monthly_rate', $manpower->monthly_rate) }}" class="form-control">
            </div>
